...
import { RusticTabTransition } from './RusticTabTransition';
...
const MOBILE_NAV_LINKS = [
  ...
  { name: 'CARTA', href: '#carta-digital', type: 'dashboard', colorClass: 'text-[#C900FF] hover:text-white' },
  { name: 'RESERVA', href: '#reserva', type: 'reserva' },
  ...
  { name: 'SÍGUENOS', href: '#redes', type: 'social' },
];
...
type ViewType = 'landing' | 'dashboard' | 'social' | 'reserva' | 'nosotros' | 'momentos';
...
export default function App() {
  ...
  const [transitionTarget, setTransitionTarget] = useState<string>('landing');
  ...
  // Transición Rústica Impecable entre Vistas
  const switchView = (targetView: ViewType, targetHash?: string) => {
    if (currentView === targetView && !isTransitioning) {
      if (targetHash) window.location.hash = targetHash;
      window.scrollTo({ top: 0, behavior: 'smooth' });
      return;
    }

    setTransitionTarget(targetView);
  ...
  const goToDashboard = () => switchView('dashboard', 'carta-digital');
  const goToLanding = () => switchView('landing');
  const goToSocial = () => switchView('social', 'redes');
  const goToReservation = () => switchView('reserva', 'reserva');
  const goToNosotros = () => switchView('nosotros', 'nosotros');
  const goToMomentos = () => switchView('momentos', 'momentos');
  ...
  useEffect(() => {
    if (currentView === 'dashboard') {
      ...
      runBoot();
      const t1 = setTimeout(runBoot, 60);
      const t2 = setTimeout(runBoot, 220);
      return () => {
        clearTimeout(t1);
        clearTimeout(t2);
      };
  ...
  return (
    <div className="min-h-screen font-sans selection:bg-[#C900FF] selection:text-white bg-[#050505] text-[#F5F5F5] relative overflow-x-hidden">
      
      {/* Componente de Transición Rústica Impecable */}
      <RusticTabTransition 
        isTransitioning={isTransitioning} 
        viewName={transitionTarget} 
      />

      {/* Orquestador de Vistas / Pestañas con AnimatePresence */}
      <AnimatePresence mode="wait" initial={false}>
        ...
        {currentView === 'nosotros' && (
          <motion.div
            ...
            exit={{ opacity: 0, y: -12, filter: 'blur(4px)' }}
            transition={{ duration: 0.32, ease: [0.22, 1, 0.36, 1] }}
            className="min-h-screen bg-[#050505] text-[#f5f5f5] font-sans selection:bg-[#c900ff] selection:text-white flex flex-col justify-between tab-view-container"
          >
        ...
        {currentView === 'momentos' && (
          <motion.div
            ...
            exit={{ opacity: 0, y: -12, filter: 'blur(4px)' }}
            transition={{ duration: 0.32, ease: [0.22, 1, 0.36, 1] }}
            className="min-h-screen bg-[#08080c] text-[#f5f5f5] font-sans selection:bg-[#c900ff] selection:text-white flex flex-col justify-between tab-view-container"
          >
            <header className="sticky top-0 z-50 bg-black/90 backdrop-blur-md py-3 px-4 border-b border-white/10">
              <div className="max-w-6xl mx-auto flex items-center justify-between gap-3">
                <div className="flex items-center gap-3">
        ...
        {/* ======================================================== */}
        {/* 4. DASHBOARD VIEW (CARTA DIGITAL)                        */}
        {/* ======================================================== */}
        {currentView === 'dashboard' && (
          <motion.div
            ...
            exit={{ opacity: 0, y: -12, filter: 'blur(4px)' }}
            transition={{ duration: 0.32, ease: [0.22, 1, 0.36, 1] }}
            className="min-h-screen bg-black text-white font-sans selection:bg-weekend-neon selection:text-black tab-view-container"
          >
        ...
        {currentView === 'reserva' && (
          <motion.div
            ...
            exit={{ opacity: 0, y: -12, filter: 'blur(4px)' }}
            transition={{ duration: 0.32, ease: [0.22, 1, 0.36, 1] }}
            className="min-h-screen bg-black text-white font-sans selection:bg-weekend-neon selection:text-black flex flex-col justify-between tab-view-container"
          >
            <header className="sticky top-0 z-50 bg-black/90 backdrop-blur-md py-3 px-4 border-b border-white/10">
          ...
          >
            ...
            >
              <div className="max-w-7xl mx-auto px-4 sm:px-6 flex items-center justify-between gap-3 sm:gap-4">
                ...
                {/* Navegación centrada con Indicador Rústico Deslizante */}
                <nav className="hidden lg:flex items-center gap-1 xl:gap-1.5 justify-center bg-black/40 px-3 py-1 rounded-full border border-white/10 backdrop-blur-md">
                  {NAV_LINKS.map((link) => {
                    ...
                    return (
                      <motion.a
                        ...
                        whileTap={{ scale: 0.96 }}
                        className={`relative px-3.5 py-1.5 rounded-full text-xs xl:text-sm uppercase tracking-wider font-extrabold transition-colors duration-200 cursor-pointer whitespace-nowrap z-10 ${
                          isActive ? 'text-black font-black' : 'text-zinc-300 hover:text-white'
                        }`}
                      ...
                      >
                        {isActive && (
                          <motion.div
                            ...
                            className="absolute inset-0 bg-gradient-to-r from-weekend-neon via-emerald-400 to-[#0acc80] rounded-full -z-10 shadow-[0_0_15px_rgba(10,204,128,0.5)]"
